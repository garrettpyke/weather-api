    ...
    return (
      <div className="weather-container">
        <div id="location-head">
          <h3>
            Weather in {currentData.location.name},{" "}
        ...
        <div className="weather weather-current">
          ...
          <img
            ...
            id="icon-current"
            src={`${urlPrefix}${currentData.current.condition.icon}`}
          />
          <h5>{currentData.current.condition.text}</h5>
          <h5>
            {currentData.current.temp_f}℉ / {currentData.current.temp_c}℃
          </h5>
          <h5>
            Wind: {currentData.current.wind_mph}mph /{" "}
            {currentData.current.wind_kph}kph ({currentData.current.wind_dir})
          </h5>
          <h5>
            Feels like: {currentData.current.feelslike_f}℉ /{" "}
        ...
        <div className="weather weather-forecast">
          ...
          <img
            className="weather-icon"
            id="forecast-icon"
            src={`${urlPrefix}${currentData.forecast.forecastday[1].day.condition.icon}`}
          />
          <h5>{currentData.forecast.forecastday[1].day.condition.text}</h5>
          <h5>
            {currentData.forecast.forecastday[1].day.avgtemp_f}℉ /{" "}
            {currentData.forecast.forecastday[1].day.avgtemp_c}℃
          </h5>
          <h5>
            Wind: {currentData.forecast.forecastday[1].day.maxwind_mph}mph /{" "}
            {currentData.forecast.forecastday[1].day.maxwind_kph}kph
          </h5>
          <h5>
            Chance of Rain/Snow:{" "}
            {currentData.forecast.forecastday[1].day.daily_chance_of_rain}% /{" "}
            {currentData.forecast.forecastday[1].day.daily_chance_of_snow}%
          </h5>
          <h5>
